...
class GPRFitter(RegressorMixin, BaseEstimator):
    ...
    def __call__(self, xVals, return_std=False, raise_warning=True):
        if raise_warning:
            if np.any(xVals < self.minVals) or np.any(xVals > self.maxVals):
                warnings.warn("Parameters " + str(xVals) + " are outside of the fit range!")
    ...
    def fit(self, xVals, yVals, yVals_std=None, normalize_y=True, n_restarts_optimizer=10, **kwargs):
        ...
        self.linear_fit(
            xVals, yVals, sample_weight=sample_weight
        )
        
        self.gpr_fit(
            xVals, yVals, alpha=alpha,
    ...
    def setup_kernel(
            ...
            constKernel_bounds=(1.e-2, 1.e2),
            noiseKernel_init=1.e-4,
            noiseKernel_bounds=(1.e-7, 1.e-2),
            lengthScale_bounds_factors=(1.e-2, 1.e1)
    ):
        ndims = xVals.shape[1]
        
        lengthScale_init = np.asarray(
            [(np.max(xVals[:, i]) 
              - np.min(xVals[:, i])) for i in range(ndims)]
        )
